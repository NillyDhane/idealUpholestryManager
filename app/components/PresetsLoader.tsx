  ...
  return (
    ...
    >
      ...
      {!loading && !error && presets.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 animate-fade-in">
          {presets.map((preset, index) => (
            <div
              key={preset.id}
              className="relative group transform transition-all duration-300 ease-in-out"
              style={{
      ...
      {showDeleteModal && selectedPreset && (
        ...
        >
          <div
            className={`bg-white rounded-lg p-6 max-w-md w-full mx-4 relative shadow-xl transition-all duration-300 ${
          ...
          >
            ...
            <div className="bg-gray-50 p-4 rounded-md mb-6">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Created:{" "}
                {new Date(selectedPreset.createdAt || "").toLocaleDateString(
                  "en-GB"
                )}
              </p>
            </div>
            <div className="flex justify-end gap-4">
              <button
                ...
                className="px-4 py-2 text-gray-600 hover:text-gray-800 hover:cursor-pointer font-medium transition-colors duration-200"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmDelete}
                className="px-4 py-2 bg-red-500 text-white font-medium rounded-md hover:bg-red-600 hover:cursor-pointer transition-colors duration-200"
              >
                Delete Preset
              </button>
            </div>
